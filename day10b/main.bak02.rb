  ...
  map.each_with_index do |row, i|
    ...
    row.each_with_index do |cell, j|
      if loop.include?([i, j])
        ...
        if crossings.odd?
          # puts "adding #{[i, j]} b/c crossings=#{crossings} when going left-to-right"
          contained << [i, j]
        end
      end
    end
    ...
    row.reverse.each_with_index do |cell, __j__|
      ...
      if loop.include?([i, j])
        ...
        if crossings.odd?
          # puts "adding #{[i, j]} b/c crossings=#{crossings} when going right-to-left"
          contained << [i, j]
        end
      end
    end
  ...
  map.transpose.each_with_index do |column, j|
    ...
    column.each_with_index do |cell, i|
      if loop.include?([i, j])
        ...
        if crossings.odd?
          # puts "adding #{[i, j]} b/c crossings=#{crossings} when going top-to-bottom"
          contained << [i, j]
        end
      end
    end
    ...
    column.reverse.each_with_index do |cell, __i__|
      ...
      if loop.include?([i, j])
        ...
        if crossings.odd?
          # puts "adding #{[i, j]} b/c crossings=#{crossings} when going bottom-to-top"
          contained << [i, j]
        end
      end
    end
...
fail unless 4 == find_contained(loop, map).size

example = DATA.read
start, map = parse_start_and_map(example)
loop = walk_from_start(start, map)
contained = find_contained(loop, map)
